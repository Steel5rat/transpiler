...
{
    public IImmutableDictionary<string, Macro> CreateMacros(Dictionary<string, List<object>> macros)
    {
        return ValidateCircularDependencies(
            macros.ToDictionary(
                    macro => macro.Key,
                    macro => new Macro(macro.Value))
                .ToImmutableDictionary());
    }

    private IImmutableDictionary<string, Macro> ValidateCircularDependencies(IImmutableDictionary<string, Macro> macros)
    {
